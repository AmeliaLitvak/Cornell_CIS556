...
def discover_violations(directory,output):
    ...
    """
    listViolations = violations.list_weather_violations(directory)                    # Implement this function
    numViolations = len(listViolations)
    if numViolations == 0:
        print('No violations found.')
    elif numViolations == 1:
        print('1 violation found.')
    else:
        print(f'{numViolations} violations found.')
    
    if output != None:
        header = ['STUDENT','AIRPLANE','INSTRUCTOR','TAKEOFF','LANDING','FILED','AREA','REASON']
        tableViolations = [header]
        for row in listViolations:
            tableViolations.append(row)
        utils.write_csv(tableViolations,output)
...
def execute(args):
    ...
    elif len(args) == 2 and '--test' in args:
        print('Usage: python auditor dataset [output.csv]')
    elif args[0] == '--test':
        tests.test_all()
    else:
