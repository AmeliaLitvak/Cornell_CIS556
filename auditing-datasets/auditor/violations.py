...
def bad_visibility(visibility,minimum):
    ...
    try:                    # Implement this function
        minVis = visibility["minimum"]
    except:
        try:
            minVis = visibility["prevailing"]
        except:
            return True
...
def bad_winds(winds,maxwind,maxcross):
    ...
    if winds == "calm":
        return False
    elif winds == "unavailable":                    # Implement this function
        return True
    else:
        try:
            recWind = winds["gusts"]
            ...
            recWind = winds["speed"]
        try:
            recCross = winds["crosswind"]
        except:
            recCross = 0

        if winds["units"] == "MPS":
            recWind = recWind*1.94384
            recCross = recCross*1.94384
        
        if (recWind > maxwind) or (recCross > maxcross):
            return True
        else:
            return False
...
def bad_ceiling(ceiling,minimum):
    ...
    if ceiling == "clear":
        return False
    elif ceiling == "unavailable":                    # Implement this function
        return True
    else:
        for layer in ceiling:
            if (layer["type"] == "broken") or (layer["type"] == "overcast") or (layer["type"] == "indefinite ceiling"):
                minHeight = layer["height"]
                break
            minHeight = 40000.0
        if minHeight < minimum:
            return True
        else:
            return False
...
def get_weather_report(takeoff,weather):
    ...
    try:
        takeoffReport = weather[isoTakeoff]
        return takeoffReport
    except:
        reportKeys = list(weather.keys())
        foundFlag = False
        for index in range(0,len(reportKeys)):
            reportTime = utils.str_to_time(reportKeys[index])
            if reportTime - takeoff < datetime.timedelta(seconds = 0):
                foundFlag = True
                break
        if foundFlag:
            foundKey = reportKeys[index]
            foundReport = weather[foundKey]
            return foundReport
        else:
            return None
...
def get_weather_violation(weather,minimums):
    ...
    """
    try:
        visTest = bad_visibility(weather["visibility"],minimums[1])                    # Implement this function
        windTest = bad_winds(weather["wind"],minimums[2],minimums[3])
        ceilTest = bad_ceiling(weather["sky"],minimums[0])

        if not (visTest or windTest or ceilTest):
            return ''
        elif visTest:
            if windTest or ceilTest:
                return 'Weather'
            else:
                return 'Visibility'
        elif windTest:
...
def list_weather_violations(directory):
    ...
    daycycleDict = utils.read_json(os.path.join(directory,DAYCYCLE))
    weatherDict = utils.read_json(os.path.join(directory,WEATHER))
    pilotList = utils.read_csv(os.path.join(directory,STUDENTS))
    minimumsList = utils.read_csv(os.path.join(directory,MINIMUMS))
    lessonsList = utils.read_csv(os.path.join(directory,LESSONS))

    
    accumulator = []
    # For each of the lessons
    for index in range(1,len(lessonsList)):
        lesson = lessonsList[index]
        # Get the takeoff time
        takeoff = utils.str_to_time(lesson[3])
        
        # Get the pilot credentials
        pilotID = utils.get_for_id(lesson[0],pilotList)
        certClass = pilots.get_certification(takeoff,pilotID)
        ...
        if lesson[5] == 'VFR':
            ...
            vfrCheck = False
        dayCheck = utils.daytime(takeoff,daycycleDict)
        minReturn = pilots.get_minimums(certClass, area, instructed, vfrCheck, dayCheck, minimumsList)
        
        # Get the weather conditions
        conditions = get_weather_report(takeoff,weatherDict)

        # Check for a violation and add to result if so
        weatherViolation = get_weather_violation(conditions,minReturn)
        if weatherViolation != '':
            lesson.append(weatherViolation)
            accumulator.append(lesson)
    return accumulator
