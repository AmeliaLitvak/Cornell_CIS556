...
def read_csv(filename):
    ...
    is a valid CSV file
    """
    file = open(filename)                    # Implement this function
    wrapper = csv.reader(file)
    accumulator = []
    for line in wrapper:
        accumulator.append(line)
    file.close()
    return accumulator                   # Implement this function
...
def write_csv(data,filename):
    ...
    """
    file = open(filename,'w')                    # Implement this function
    wrapper = csv.writer(file)
    for row in data:
        wrapper.writerow(row)
    file.close()                    # Implement this function
...
def read_json(filename):
    ...
    file.close()
    return data                    # Implement this function
    ...
    if result != None:
        if result.tzinfo == None:
            if type(tzsource) == str:
                correct = timezone(tzsource).localize(result)
                replace = str_to_time(timestamp,correct)
                result = replace
            elif type(tzsource) == datetime.datetime:
                replace = result.replace(tzinfo=tzsource.tzinfo)
                result = replace
    return result
...
def daytime(time,daycycle):
    ...
    yearKey = time.strftime('%Y')                    # Implement this function
    month = time.strftime('%m')
    day = time.strftime('%d')
    dateKey = month + '-' + day
    hour = time.strftime('%H')
    minute = time.strftime('%M')
    timeKey = 'T' + hour + ':' + minute
    try:
        yearDictionary = daycycle[yearKey]
        dateDictionary = yearDictionary[dateKey]
        sunriseTime = dateDictionary["sunrise"]
        sunsetTime = dateDictionary["sunset"]
        isosunriseTime = yearKey + '-' + dateKey + 'T' + sunriseTime
        isosunsetTime = yearKey + '-' + dateKey + 'T' + sunsetTime
        if time.tzinfo == None:
            sunriseResult = str_to_time(isosunriseTime)
            sunsetResult = str_to_time(isosunsetTime)
        else:
            sunriseResult = str_to_time(isosunriseTime,"America/New_York")
            sunsetResult = str_to_time(isosunsetTime,"America/New_York")
        
        if (time > sunriseResult) & (time < sunsetResult):
            result = True
        else:
            result = False 
    except:
...
def get_for_id(id,table):
    ...
    """
    result = None
    for row in table:                    # Implement this function
        if row[0] == id:
            result = row
            break
    return result
