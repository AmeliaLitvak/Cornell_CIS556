...
def get_certification(takeoff,student):
    ...
    """
    try:
        joinDate = utils.str_to_time(student[3],"America/New_York")
        if joinDate > takeoff:
            cert = -1
        else:
            try:
                soloDate = utils.str_to_time(student[4],"America/New_York")
                if soloDate > takeoff:
                    cert = 0
                else:
                    try:
                        licenseDate = utils.str_to_time(student[5],"America/New_York")
                        if licenseDate > takeoff:
                            cert = 1
                        else:
                            try:
                                hour50Date = utils.str_to_time(student[6],"America/New_York")
                                if hour50Date > takeoff:
                                    cert = 2
                                else:
                                    cert = 3
                            except:
...
def get_best_value(data, index, maximum=True):
    ...
    listCompare = []
    for row in data:
        listCompare.append(float(row[index]))
    if maximum:
        return max(listCompare)
    else:
        return min(listCompare)
    ...
    for row in range(1,len(minimums)):
        ...
        if cert > 0:
            if minimums[row][0] == 'Student':
                if cert >= 1:
                    certCheck = True
            elif minimums[row][0] == 'Certified':
                if cert >= 2:
                    certCheck = True
            elif minimums[row][0] == '50 Hours':
                if cert == 3:
                    certCheck = True
            elif minimums[row][0] == 'Dual':
                if instructed:
                    certCheck = True
        else:
        ...
        if minimums[row][2] == area:
            areaCheck = True
        elif minimums[row][2] == 'Local':
            if area == 'Pattern' or area == 'Practice Area':
                areaCheck = True
        elif minimums[row][2] == 'Any':
            areaCheck = True

        if minimums[row][1] == 'VMC':
            if vfr:
                vfrCheck = True
        elif minimums[row][1] == 'IMC':
            if not vfr:
                vfrCheck = True
        ...
        if minimums[row][3] == 'Day':
            if daytime:
                dayCheck = True
        elif minimums[row][3] == 'Night':
            if not daytime:
                dayCheck = True

        if (certCheck & areaCheck & vfrCheck & dayCheck):
            accumulator.append(minimums[row])

    try:
        ceilMin = get_best_value(accumulator,4,False)
        visMin = get_best_value(accumulator,5,False)
        windMax = get_best_value(accumulator,6,True)
        crosswindMax = get_best_value(accumulator,7,True)
        return [ceilMin, visMin, windMax, crosswindMax]
    except:
        return None